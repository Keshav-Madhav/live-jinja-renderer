...
- **Copilot Button**: New button (Copilot icon) in Variables header - only appears if Copilot is installed
- **Contextual Generation**: Copilot analyzes your template and generates semantically appropriate values
- **Streaming Response**: Watch values stream in real-time as Copilot generates them
- **Visual Feedback**: 
  - Rotating border animation during generation
  - "Asking Copilot..." / "Generated by Copilot ✓" toast notifications
  - Animated gradient on textarea while streaming
- **Smart Fallback**: If Copilot is unavailable, use the algorithmic ✦ button instead
